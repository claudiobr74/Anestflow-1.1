/**
 * Vocabulário de transcrição anestésica.
 * Nomes do catálogo canônico + termos pedidos na arquitetura.
 * Sem doses, diluições ou vias padrão.
 */

const CANONICAL_DRUG_NAMES = [
  "Propofol",
  "Etomidato",
  "Cetamina",
  "Midazolam",
  "Tiopental",
  "Dexmedetomidina",
  "Diazepam",
  "Rocurônio",
  "Atracúrio",
  "Cisatracúrio",
  "Succinilcolina",
  "Pancurônio",
  "Fentanil",
  "Sufentanil",
  "Remifentanil",
  "Alfentanil",
  "Morfina",
  "Tramadol",
  "Nalbufina",
  "Adrenalina",
  "Noradrenalina",
  "Dobutamina",
  "Dopamina",
  "Milrinona",
  "Efedrina",
  "Metaraminol",
  "Fenilefrina",
  "Vasopressina",
  "Atropina",
  "Amiodarona",
  "Esmolol",
  "Clonidina",
  "Hidralazina",
  "Adenosina",
  "Metoprolol",
  "Nitroglicerina",
  "Nitroprussiato de Sódio",
  "Ondansetrona",
  "Dexametasona",
  "Metoclopramida",
  "Droperidol",
  "Cefazolina",
  "Lidocaína",
  "Dipirona",
  "Cetoprofeno",
  "Cetorolaco",
  "Parecoxibe",
  "Tenoxicam",
  "Sugamadex",
  "Neostigmina",
  "Flumazenil",
  "Naloxona",
  "Furosemida",
  "Gluconato de Cálcio",
  "Hidrocortisona",
  "Bicarbonato de Sódio",
  "Ácido Tranexâmico",
  "Tranexâmico",
  "Ocitocina",
  "Insulina Regular",
  "Bupivacaína",
  "Ropivacaína",
  "Sevoflurano",
  "Desflurano",
  "Isoflurano",
  "Óxido Nitroso",
  "Oxigênio",
  "Ar comprimido",
] as const;

/** Jargão falado — só para reconhecimento ASR, não para inferir dose/via. */
const SPOKEN_ALIASES = [
  "fenta",
  "remi",
  "sufenta",
  "nora",
  "norinha",
  "sevo",
  "des",
  "iso",
  "keta",
  "rocu",
  "esmeron",
  "nimbium",
  "precedex",
  "adrena",
  "atropo",
] as const;

export const DRUG_GROUNDING_ALIASES: Record<string, readonly string[]> = {
  fentanil: ["fentanil", "fenta"],
  sufentanil: ["sufentanil", "sufenta"],
  remifentanil: ["remifentanil", "remi"],
  propofol: ["propofol"],
  etomidato: ["etomidato"],
  cetamina: ["cetamina", "ketamina", "keta"],
  dexmedetomidina: ["dexmedetomidina", "precedex", "dexmede"],
  rocurônio: ["rocuronio", "rocurônio", "rocu", "esmeron"],
  cisatracúrio: ["cisatracurio", "cisatracúrio", "nimbium", "cis"],
  sugamadex: ["sugamadex", "sugammadex"],
  neostigmina: ["neostigmina"],
  atropina: ["atropina", "atropo"],
  efedrina: ["efedrina"],
  metaraminol: ["metaraminol"],
  noradrenalina: ["noradrenalina", "nora", "norinha", "norepinefrina"],
  adrenalina: ["adrenalina", "adrena", "epinefrina"],
  fenilefrina: ["fenilefrina"],
  vasopressina: ["vasopressina"],
  sevoflurano: ["sevoflurano", "sevo", "sevorane"],
  desflurano: ["desflurano", "des", "suprane"],
  isoflurano: ["isoflurano", "iso", "forane"],
  ondansetrona: ["ondansetrona"],
  dexametasona: ["dexametasona"],
  dipirona: ["dipirona"],
  cetorolaco: ["cetorolaco", "ketorolaco"],
  tranexâmico: ["tranexamico", "tranexâmico", "acido tranexamico"],
  midazolam: ["midazolam"],
  lidocaína: ["lidocaina", "lidocaína"],
};

export function canonicalAnesthesiaTerms(): string[] {
  return [...CANONICAL_DRUG_NAMES];
}

/** Lista enviada ao modelo de transcrição (verbatim). Sem doses. */
export function transcriptionVocabulary(limit = 100): string[] {
  const seen = new Set<string>();
  const out: string[] = [];
  for (const term of [...CANONICAL_DRUG_NAMES, ...SPOKEN_ALIASES]) {
    const key = term.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
    if (seen.has(key)) continue;
    seen.add(key);
    out.push(term);
    if (out.length >= limit) break;
  }
  return out;
}
