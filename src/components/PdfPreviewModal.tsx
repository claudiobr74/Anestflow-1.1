...
export default function PdfPreviewModal({
  ...
  return (
    <div className={`fixed inset-0 z-[9999] flex flex-col font-sans animate-fade-in select-none ${
      isDark ? "bg-[#0F1113] text-white" : "bg-slate-100 text-slate-900"
    }`}>
      {/* TOOLBAR — chrome segue o tema; as folhas do PDF continuam brancas para impressão */}
      <header className={`min-h-[56px] flex flex-wrap items-center justify-between gap-4 px-6 py-2.5 border-b shrink-0 ${
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:block">
            <h1 className={`text-xs font-black tracking-tight uppercase tabular-nums ${
              isDark ? "text-zinc-200" : "text-slate-800"
            }`}>
              {patient.fullName || "Ficha Sem Nome"}
            </h1>
            <p className={`text-xs font-bold uppercase tabular-nums tracking-wider ${
        ...
        <div className="flex flex-1 sm:flex-none items-center justify-center gap-2">
          <button onClick={() => setScale(s => Math.max(0.2, s - 0.1))} className={`p-1.5 rounded ${
            isDark ? "bg-zinc-800 hover:bg-zinc-700 text-zinc-300" : "bg-slate-100 hover:bg-slate-200 text-slate-700"
          }`}>
            <ZoomOut className="w-3.5 h-3.5" />
          </button>
          <span className={`text-xs tabular-nums w-12 text-center ${isDark ? "text-zinc-400" : "text-slate-500"}`}>{Math.round(scale * 100)}%</span>
          <button onClick={() => setScale(s => Math.min(2, s + 0.1))} className={`p-1.5 rounded ${
            isDark ? "bg-zinc-800 hover:bg-zinc-700 text-zinc-300" : "bg-slate-100 hover:bg-slate-200 text-slate-700"
          }`}>
            <ZoomIn className="w-3.5 h-3.5" />
          </button>
          <button onClick={() => setScale(window.innerWidth < 1123 ? (window.innerWidth - 32) / 1123 : 1)} className={`p-1.5 rounded ${
            isDark ? "bg-zinc-800 hover:bg-zinc-700 text-zinc-300" : "bg-slate-100 hover:bg-slate-200 text-slate-700"
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className={`w-px h-5 mx-1 ${isDark ? "bg-zinc-800" : "bg-slate-200"}`} />
      ...
      {/* VIEWER ENGINE AREA */}
      <main className={`flex-1 relative overflow-auto p-4 md:p-8 flex flex-col items-center justify-start select-text ${
        isDark ? "bg-[#0A0C0D]" : "bg-slate-200"
      }`}>
        <div 
          ref={containerRef} 
