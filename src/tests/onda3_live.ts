...
import { saveProcedure, getProcedures, getProcedureById } from "../lib/proceduresService.ts";
import { saveToWorklist, getFromWorklist, hashCpf } from "../lib/worklistService.ts";
import { getBlankDocument } from "../mockData.ts";
import { signAndLockDocument } from "../lib/signatureService.ts";
...
blank.userId = uid;
blank.patient.fullName = "Paciente Teste Onda Tres";
blank.patient.cpf = cpf;
blank.patient.recordNumber = "ONDA3-001";
...
if (!worklist || worklist.patient.fullName !== "Paciente Teste Onda Tres") fail("worklist roundtrip");
...
if (!reloaded || reloaded.patient.fullName !== "Paciente Teste Onda Tres") fail("getProcedureById");

const signed = await signAndLockDocument(reloaded, {
  uid,
  name: "Dr. Agente Onda Tres",
  crm: "123456",
  uf: "GO"
});
await saveProcedure(signed, uid);
const afterSign = await getProcedureById(blank.id);
